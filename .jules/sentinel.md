...
## 2024-05-18 - [Timing Attack via Empty Secret]
**Vulnerability:** In `AuthenticateClient`, an empty `clientSecret` caused an early return without performing a hash comparison, allowing an attacker to differentiate between invalid client IDs (which triggered a dummy comparison) and valid confidential client IDs with empty secrets (which returned quickly).
**Learning:** Input validation (like checking for an empty secret) that occurs *after* a database lookup can inadvertently introduce timing side-channels if it bypasses expensive operations that are normally performed.
**Prevention:** Always ensure that execution paths take a consistent amount of time for a given client ID, regardless of whether subsequent validation checks (like secret format or length) succeed or fail, by adding dummy operations where necessary.
