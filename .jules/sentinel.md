...
## 2025-02-27 - [Fix Timing Attack Client Enumeration via Empty Secret Check]
**Vulnerability:** In `AuthenticateClient` (`exchange.go`), for confidential clients, an initial input validation check for an empty `req.ClientSecret` occurs early, skipping the actual cryptographic comparison.
**Learning:** Returning immediately on early secret input validation checks after retrieving the client from the database allows attackers to enumerate valid client IDs by measuring timing differences (which clients return fast vs slow).
**Prevention:** Always pair input validation checking for an empty secret with a dummy cryptographic operation (like `hasher.DummyCompare(ctx)`) to ensure timing consistency regardless of early validation results.
