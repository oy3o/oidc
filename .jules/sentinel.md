...
## 2024-05-24 - [Timing Attack via Empty Secret Check]
**Vulnerability:** In `AuthenticateClient`, checking for an empty `clientSecret` after the database lookup returned early without a dummy hash comparison. This allowed attackers to enumerate valid client IDs by sending an empty secret, which returned significantly faster than providing an invalid client ID (which correctly triggered a DummyCompare).
**Learning:** Input validation that occurs *after* a database lookup must be time-balanced with dummy operations if the expected normal path involves expensive operations like password hashing. Returning early on invalid input after a DB lookup creates a timing oracle.
**Prevention:** Always pair input validation (like empty secret checks) that occurs after a database lookup with dummy cryptographic operations (e.g., `DummyCompare`) before returning to prevent timing attacks and client enumeration.
