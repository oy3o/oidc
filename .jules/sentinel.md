...
## 2026-04-02 - [Client ID Enumeration via Timing Discrepancy]
**Vulnerability:** The token exchange endpoints returned early when the client secret was empty after validating the client ID, skipping the expensive hash validation. This allowed attackers to enumerate valid client IDs by observing the response time difference between valid and invalid client IDs when providing an empty secret.
**Learning:** Constant-time responses must be maintained even when early validation (such as checking for an empty secret) fails, *if* that validation happens after a database lookup confirms the existence of the entity.
**Prevention:** Ensure constant-time responses by performing dummy cryptographic operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped after the entity has been verified to exist.