...
## 2024-05-24 - [Bypass of DPoP Cryptographic Validation / DoS Risk]
**Vulnerability:** DPoP headers were reparsed using `ParseUnverified` to extract the `ATH` claim in `DPoPMiddleware`, bypassing cryptographic validation of the token claims if the JWT structure was manipulated after the valid signature block. Additionally, unbounded DPoP header lengths could lead to memory/CPU DoS attacks during parsing.
**Learning:** Always use the claims object returned by the cryptographic verification process (`ParseWithClaims`) instead of relying on `ParseUnverified`. Unverified parsing allows attackers to append arbitrary payload data to a valid token if the parser is flawed or if multiple tokens are presented. Further, cryptographic inputs should have strict length limits enforced before processing to prevent resource exhaustion.
**Prevention:** Remove all uses of `jwt.ParseUnverified` for token claims extraction. Ensure that the verification function (`VerifyDPoPProof`) directly returns the validated claims object (`*DPoPProof`) alongside the JKT. Enforce a length limit (e.g., 4096 bytes) on the DPoP header string before passing it to the JWT parser.
