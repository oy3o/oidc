...
## 2026-04-02 - [Timing Attack / Client Enumeration in AuthenticateClient]
**Vulnerability:** In `exchange.go`, `AuthenticateClient` performed an early return when a confidential client was provided an empty `client_secret` without doing a dummy hash comparison. This allowed attackers to enumerate confidential clients quickly without paying the hashing cost.
**Learning:** This is a classic timing attack vulnerability (CWE-208 / CWE-203). Even if a secret is structurally invalid (empty), if we are evaluating a valid client, we must ensure the response time is constant to hide the existence of the client. The fix is to call `hasher.DummyCompare(ctx)` before returning the error.
**Prevention:** Whenever skipping a real hash comparison for a valid entity (like early validation of empty credentials), always perform a dummy hash operation to maintain constant-time responses and prevent enumeration attacks.
