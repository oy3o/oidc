...
## 2024-04-04 - [Client ID Enumeration via Timing Attack]
**Vulnerability:** In `AuthenticateClient`, when a confidential client was found but the provided `clientSecret` was empty, the function returned immediately without performing a dummy hash comparison.
**Learning:** This allowed an attacker to enumerate valid `client_id`s by sending an empty `client_secret` and measuring the response time. A valid `client_id` with an empty secret would fail instantly, while an invalid `client_id` would trigger a `DummyCompare` (taking ~50ms).
**Prevention:** To prevent client ID enumeration attacks, authorization and token exchange endpoints must ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped, such as when an empty secret is provided for a valid confidential client.
