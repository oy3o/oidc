...
## 2024-06-26 - [CRITICAL] Fix DPoP JWT Parsing Input DoS and Claims Forgery
**Vulnerability:** DPoP headers were parsed using `jwt.ParseWithClaims` without length limits, and `dpop_middleware.go` re-parsed the JWT using `ParseUnverified` to extract the `ATH` (Access Token Hash) claim instead of securely using the verified claims.
**Learning:** 1. Missing length limits on cryptographic inputs risks DoS (CPU/Memory exhaustion). 2. Re-parsing a token using `ParseUnverified` to fetch additional claims after an initial verification step bypasses signature guarantees and allows an attacker to manipulate those claims.
**Prevention:** Always enforce strict length limits on unverified JWT inputs before parsing. Always extract all needed claims during the cryptographic verification step (e.g., from the return of `VerifyDPoPProof`) rather than re-parsing with `ParseUnverified`.
