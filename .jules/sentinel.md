...
## 2024-05-18 - [Timing Attack via Empty Client Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client provides an empty secret, the function returns immediately with an error, skipping the expensive password hash validation.
**Learning:** This discrepancy in processing time allows an attacker to easily distinguish whether a client requires a secret or if a specific client ID corresponds to a valid confidential client. To mask the presence of valid user IDs, the time spent evaluating must be consistent regardless of input correctness.
**Prevention:** Always perform a dummy comparison (e.g., `hasher.DummyCompare(ctx)`) whenever skipping an actual password validation, ensuring constant-time validation across successful, invalid, and missing credential paths.
