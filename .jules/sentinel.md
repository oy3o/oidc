...
## 2024-05-29 - [Timing Oracle in AuthenticateClient Secret Validation]
**Vulnerability:** The empty secret check in `AuthenticateClient` returned immediately without executing a dummy cryptographic operation, creating a timing oracle.
**Learning:** Timing differences between early validation checks (like empty secret) and full hash comparisons can allow attackers to enumerate valid client IDs.
**Prevention:** When input validation (such as checking for an empty secret) occurs after a database lookup, it must be paired with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to prevent timing attacks and client enumeration.
