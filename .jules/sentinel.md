...
## 2024-05-24 - [Timing Attack via Empty Client Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client provided an empty `clientSecret`, the function returned immediately without performing any dummy hash operations (`hasher.DummyCompare(ctx)`).
**Learning:** This fast failure allowed attackers to distinguish between valid and invalid client IDs by measuring response times when an empty secret was supplied. A fast response indicated the client ID was valid (but the secret was empty), while a slow response (due to the dummy hash on the `ErrInvalidClient` path) indicated the client ID was invalid.
**Prevention:** Always perform a dummy hash comparison before returning an error in authentication flows when skipping the real hash check to normalize response times and prevent ID enumeration.
