...
## 2024-05-20 - [Client ID Enumeration via Missing Dummy Hash Compare]
**Vulnerability:** An early return in `AuthenticateClient` for an empty `client_secret` (after a valid client ID was successfully looked up) bypassed the cryptographic hash comparison, allowing attackers to perform a timing attack to enumerate valid confidential client IDs.
**Learning:** When input validation (like checking for an empty secret) is performed *after* a successful database lookup, returning early without a dummy hash comparison introduces a timing discrepancy compared to an invalid client lookup (which triggers `DummyCompare`).
**Prevention:** Always pair input validation failures that occur *after* a successful database lookup with a dummy cryptographic operation (e.g., `DummyCompare`) before returning the error.
