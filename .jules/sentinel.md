...
## 2026-06-14 - [DPoP Header Length Limit]
**Vulnerability:** The DPoP header parsing logic (`jwt.ParseWithClaims`) processed `dpopHeader` directly without checking its length. Since DPoP headers are often parsed using CPU-intensive operations (like validating signatures or checking formats), excessively large `DPoP` headers could be used to mount a CPU/Memory Denial of Service (DoS) attack.
**Learning:** Whenever an HTTP header is accepted as input for cryptographic parsing or decoding (such as JWTs), we must restrict the input size *before* passing it to parsing functions, to avoid excessive memory allocation and computation.
**Prevention:** Enforce a strict length limit (e.g., 4096 bytes) on the raw token strings extracted from HTTP headers (like DPoP) before passing them to parsing routines like `jwt.ParseWithClaims`.
