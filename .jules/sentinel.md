...
**Prevention:** Enforce input length bounds (e.g., maximum token length) *before* decoding strings and *always* check for and gracefully handle `err` returned by `DecodeString` routines.## 2025-04-30 - Client Enumeration via Empty Secret Timing Leak
**Vulnerability:** The `AuthenticateClient` function in `exchange.go` returned early when checking if `clientSecret == ""` without executing a dummy hash comparison. This allowed attackers to differentiate between an empty secret (fast failure) and an invalid secret (slow failure due to hashing), potentially facilitating client ID enumeration.
**Learning:** Even early, non-cryptographic validation checks (like empty string checks) can leak timing information if they bypass expensive cryptographic operations that would otherwise occur.
**Prevention:** Always execute a dummy hashing operation (e.g., `hasher.DummyCompare(ctx)`) in failure paths that skip the primary secret validation logic to maintain a constant response time.
