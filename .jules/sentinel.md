...
## 2026-03-24 - [DPoP Header Length Limit Missing]
**Vulnerability:** The `VerifyDPoPProof` function parses the DPoP HTTP header directly into a JWT struct using `jwt.ParseWithClaims` without first enforcing a maximum length. This opens the application up to a CPU and memory exhaustion Denial of Service (DoS) attack, as an attacker could send excessively large headers.
**Learning:** Cryptographic and parsing operations, including JWT parsing, must not process unbounded input lengths to prevent resource exhaustion attacks. Large inputs can cause spikes in CPU/memory usage leading to application instability.
**Prevention:** Enforce input length bounds (e.g., maximum token or header length) *before* decoding, parsing or operating on strings that come from user inputs or HTTP headers.