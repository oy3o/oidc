...
## 2025-02-28 - [Timing Attack via Empty Secret Check]
**Vulnerability:** In `AuthenticateClient`, an empty `clientSecret` for a confidential client triggered an early return *after* the client was retrieved from the database, bypassing the hashing step.
**Learning:** Checking for an empty secret after a database lookup creates a timing oracle. Attackers can provide an empty secret to rapidly determine if a client ID exists and is confidential, negating other timing attack protections (like dummy compares for invalid clients).
**Prevention:** Always pair input validation that occurs *after* a database lookup (like empty secret checks) with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to maintain constant execution time.
