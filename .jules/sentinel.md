...
## 2026-04-13 - [Timing Attack via Empty Client Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client provided an empty `clientSecret`, the code immediately returned an error without performing a hash comparison (`hasher.DummyCompare()`), creating a timing difference between a request with a valid client ID but empty secret and a request with an invalid client ID.
**Learning:** This discrepancy allows an attacker to enumerate valid Client IDs by observing the response time. The application was properly mitigating timing attacks for invalid Client IDs using `DummyCompare`, but failed to maintain the constant-time guarantee when the secret was empty.
**Prevention:** Ensure constant-time response across *all* invalid authentication paths. Whenever a real cryptographic validation is bypassed (e.g. empty inputs or early failure), execute the equivalent dummy workload (`Hasher.DummyCompare`).
