...
## 2024-06-28 - [DoS via Unbounded DPoP Header Parsing]
**Vulnerability:** The `VerifyDPoPProof` function extracted the `DPoP` header from incoming requests and passed it directly to `jwt.ParseWithClaims` without any length checks.
**Learning:** Parsing an arbitrarily large JWT string can lead to significant CPU and memory exhaustion, resulting in Denial of Service (DoS) attacks, especially since DPoP is used on highly accessed endpoints.
**Prevention:** Always enforce strict length bounds (e.g., maximum token length) on all user-supplied headers, especially those passed into cryptographic or complex parsing functions like JWT processing.
