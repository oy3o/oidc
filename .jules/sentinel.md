...
## 2026-03-22 - [Bypass of DPoP Cryptographic Validation]
**Vulnerability:** The DPoP middleware `DPoPMiddleware` parsed the DPoP Proof token a second time using `jwt.ParseUnverified` to extract claims like the Access Token Hash (ATH), bypassing the cryptographic verification that had just been performed.
**Learning:** Using `ParseUnverified` on security tokens allows attackers to bypass validation if token data is manipulated after signature generation (or if the unverified parser behaves differently). Data used for authorization decisions MUST come strictly from the returned, validated claims object produced by the cryptographic verification function (`jwt.ParseWithClaims`).
**Prevention:** Always rely on the claims object returned by the signature validation process. Modify verification routines to return the validated claims explicitly rather than re-parsing headers or tokens using unverified methods.
