...
## 2024-10-25 - [DPoP ParseUnverified Validation Bypass]
**Vulnerability:** DPoP middleware used `jwt.ParseUnverified` to extract the `ATH` (Access Token Hash) from the DPoP header after it was cryptographically validated in `VerifyDPoPProof`. While the signature was checked, re-parsing without validation allows an attacker to manipulate token claims in flight without breaking the signature in case of proxy modification, or bypass validation completely if validation logic is separated from usage.
**Learning:** Never use `ParseUnverified` to extract token claims even if the token has been verified earlier. The claims object returned by the cryptographic verification process (`jwt.ParseWithClaims`) should be passed along and used to guarantee the claims accurately reflect the verified payload and to prevent subtle validation bypasses.
**Prevention:** Always rely on the strongly-typed claims object populated during `jwt.ParseWithClaims` or the core token verification function instead of re-parsing tokens with `ParseUnverified`.
