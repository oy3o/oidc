...
## 2025-06-04 - [JWT Validation Bypass via ParseUnverified]
**Vulnerability:** The application was using `jwt.ParseUnverified` to extract the `ATH` (Access Token Hash) claim from DPoP proofs after validating them. If an attacker maliciously structured a token, `ParseUnverified` might parse claims differently than `ParseWithClaims`, leading to validation bypass.
**Learning:** Using `ParseUnverified` after cryptographic verification can create a mismatch between the verified claims and the parsed claims. Always rely on the claims object directly returned by the cryptographic verification process (`jwt.ParseWithClaims`).
**Prevention:** Never use `jwt.ParseUnverified` to extract claims for security-critical logic. Pass the claims object returned during signature validation throughout the application.
