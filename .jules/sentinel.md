...
## 2024-06-04 - Fix timing attack vulnerability in client authentication
**Vulnerability:** The `AuthenticateClient` function returns early when a confidential client provides an empty secret without performing a dummy hash comparison.
**Learning:** When an input validation (such as checking for an empty secret) occurs after a database lookup, it must be paired with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to prevent timing attacks and client enumeration.
**Prevention:** Always perform dummy cryptographic operations when skipping real hash comparisons due to invalid inputs like an empty secret.
