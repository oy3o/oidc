...
**Prevention:** Enforce input length bounds (e.g., maximum token length) *before* decoding strings and *always* check for and gracefully handle `err` returned by `DecodeString` routines.## 2026-03-27 - [Timing Attack / Client ID Enumeration via Fast Fail]
**Vulnerability:** The token exchange endpoint (`AuthenticateClient` in `exchange.go`) returned immediately when a `client_id` was not found (`ErrInvalidClient`) or when a confidential client provided an empty `client_secret`.
**Learning:** Returning early allows an attacker to measure response latency and enumerate valid `client_id`s by observing which requests take longer (because they trigger a slow password hash comparison).
**Prevention:** To ensure constant-time responses, perform a dummy hashing operation (`Hasher.DummyCompare`) whenever a real hash comparison is skipped due to invalid client ID or missing secret. The dummy operation must execute exactly one hashing workload using a pre-computed valid hash to avoid double work (generating a hash and then comparing), which could lead to CPU exhaustion DoS attacks.
