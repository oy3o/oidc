...
## 2026-06-15 - [Bypassing Cryptographic Validation with ParseUnverified]
**Vulnerability:** DPoP headers were parsed using `jwt.ParseUnverified` in `DPoPMiddleware` to extract claims (like `ATH`), bypassing cryptographic signature validation. Even though `VerifyDPoPProof` was called previously, it only returned the `JKT` and didn't return the verified claims object.
**Learning:** Using `ParseUnverified` on user-provided JWTs is dangerous because it ignores the signature. If the return value of a cryptographically verified function isn't used, attackers could potentially manipulate the unverified parsed claims.
**Prevention:** Never use `ParseUnverified` to extract data from a token. Always modify the verification function (e.g., `VerifyDPoPProof`) to return the cryptographically verified claims object and use that directly.
