...
## 2024-05-18 - [Timing Attack via Empty Secret Check After DB Lookup]
**Vulnerability:** In `exchange.go`'s `AuthenticateClient`, an early return was triggered when a confidential client was provided an empty secret. Because this check occurred *after* the client was loaded from the database and before the expensive hash verification, an attacker could observe the timing difference to reliably enumerate valid confidential client IDs.
**Learning:** Returning early on validation errors (like an empty secret) *after* an initial database lookup skips the typical heavy cryptographic workload (e.g., bcrypt comparison). This discrepancy allows timing attacks. Dummy operations (e.g., `DummyCompare`) must be executed to normalize response times whenever the expected cryptographic work is bypassed.
**Prevention:** Always pair input validation failures (like empty secrets) with a `DummyCompare` operation if the validation happens after a database lookup and before the actual hash verification.
