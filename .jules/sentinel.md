...
## 2024-06-29 - [DPoP Header Length Limit Missing]
**Vulnerability:** The DPoP header, which is parsed as a JWT via `jwt.ParseWithClaims`, had no length limits. This could allow an attacker to send an excessively large DPoP header, leading to CPU and memory exhaustion (DoS) during parsing and cryptographic operations.
**Learning:** Cryptographic and parsing operations must not process unbounded input lengths to prevent resource exhaustion.
**Prevention:** Always enforce a strict length limit (e.g., 4096 bytes) on DPoP headers before passing them to any JWT parsing functions.
