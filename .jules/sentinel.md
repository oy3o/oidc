...
**Vulnerability:** In `exchange.go`, the `AuthenticateClient` function returned immediately if a confidential client had an empty `clientSecret`, skipping the expensive hashing operation (`client.ValidateSecret`). This created a timing side-channel that allowed an attacker to enumerate valid confidential client IDs by measuring the fast response time.
**Learning:** To prevent client ID enumeration attacks, authentication flows must ensure constant-time responses. If a real hash comparison is skipped (e.g., due to an empty secret or early validation failure), a dummy cryptographic operation must be performed to mask the difference.
**Prevention:** Always use `hasher.DummyCompare(ctx)` before returning early on `ErrInvalidClient` or empty secrets after a database lookup.
