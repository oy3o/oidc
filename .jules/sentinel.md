...
## 2024-05-20 - [Client Enumeration via Timing Attack in Client Authentication]
**Vulnerability:** In `AuthenticateClient` (`exchange.go`), the check for an empty secret on a confidential client returned immediately without performing any hashing or dummy operations. This occurred *after* the client lookup in the database.
**Learning:** If an input validation check (like checking for an empty secret) allows an early return *after* a database lookup, it creates a timing discrepancy. An attacker can measure the response time. A fast response means the secret was empty but the client exists. A slow response means the client doesn't exist (because the "client not found" path correctly executed a `DummyCompare`). This difference allows an attacker to enumerate valid `client_id`s.
**Prevention:** When an early validation failure occurs *after* a database lookup, it must be paired with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to ensure constant-time execution and prevent timing-based enumeration attacks.
