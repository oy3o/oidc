...
## 2024-04-18 - [Client Enumeration Timing Attack]
**Vulnerability:** In `AuthenticateClient`, if an empty `clientSecret` is provided for a confidential client, the code returns an error immediately. If a wrong non-empty secret is provided, it goes through a computationally expensive hash verification process in `ValidateSecret`. This timing discrepancy enables an attacker to deduce whether a `clientID` exists and belongs to a confidential client based on the response time.
**Learning:** Returning early without executing a dummy hash operation when skipping expensive real hash checks creates a timing side-channel. This allows attackers to enumerate valid usernames or client IDs by comparing the response time of inputs with known empty/invalid passwords versus arbitrary non-empty passwords.
**Prevention:** Whenever a real hash comparison is skipped for an entity that usually requires one (like an early exit for an empty password or missing user/client), execute a dummy hash comparison (e.g., `DummyCompare`) to ensure the total response time remains relatively constant.
