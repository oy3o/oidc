...
## 2026-03-22 - [DoS via Unbounded DPoP Header Parsing]
**Vulnerability:** The DPoP header extracted from requests in `VerifyDPoPProof` was passed to `jwt.ParseWithClaims` without any length limits, opening a vector for CPU and memory exhaustion (DoS).
**Learning:** Parsing extremely large, maliciously crafted JWTs can consume significant resources. Input data size constraints must be enforced *before* any cryptographic validation or JWT parsing occurs.
**Prevention:** Always enforce a strict length limit (e.g., 4096 bytes) on incoming DPoP headers (and any other unvalidated token inputs) before attempting to decode or parse them.
