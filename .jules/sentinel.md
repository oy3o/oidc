...
## 2024-05-18 - [DPoP Header Size Limit / DoS Prevention]
**Vulnerability:** DPoP headers were parsed directly with `jwt.ParseWithClaims` without prior length checking, which could expose the application to CPU and memory exhaustion (DoS) via excessively large tokens.
**Learning:** Cryptographic and parsing operations (like JWT decoding) should never process unbounded input lengths, especially for unauthenticated or externally provided inputs like DPoP headers.
**Prevention:** Always enforce strict input length bounds (e.g., 4096 bytes) on DPoP headers and similar cryptographic inputs before passing them to parsing functions.
