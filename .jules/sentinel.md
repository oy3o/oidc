...
## 2026-03-21 - [Client ID Enumeration via Timing Attack]
**Vulnerability:** In `exchange.go`'s `AuthenticateClient` function, when a valid confidential client was accessed with an empty `clientSecret`, the code returned `ErrInvalidClient` immediately. This created a timing difference compared to providing an incorrect secret or an invalid client ID (which triggered a slow DummyCompare operation), allowing an attacker to confirm if a confidential client ID exists.
**Learning:** Returning early on an empty client secret after looking up the client creates an observable timing difference. You must ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped for invalid or missing credentials.
**Prevention:** Always add a dummy hash comparison (`hasher.DummyCompare(ctx)`) before returning an authentication failure if the legitimate hash verification flow was skipped.
