...
## 2024-05-28 - Unverified JWT Claims Extracted via ParseUnverified
**Vulnerability:** The DPoP verification logic in `dpop_middleware.go` was extracting claims using `new(jwt.Parser).ParseUnverified(dpopHeader, &proof)` after verification. This bypassed the security guarantees of the signature, making it theoretically possible (though mitigated by earlier steps) for an attacker to craft a payload that parsed out differently or extracted unverified claims.
**Learning:** `jwt.ParseUnverified` is a dangerous function that completely bypasses signature and algorithm validation. When using `jwt.ParseWithClaims`, it returns a populated, verified claims object, which should be reused rather than parsing the raw token twice.
**Prevention:** Always use the populated claims object returned by `jwt.ParseWithClaims`. Never use `ParseUnverified` unless strictly examining tokens pre-validation (e.g. inspecting headers for key ID) where the output is never trusted for business logic.
