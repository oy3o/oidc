...
**Vulnerability:** In `AuthenticateClient`, an empty `clientSecret` caused an early return without performing a dummy cryptographic operation, whereas a non-existent client performed a `DummyCompare`. This timing difference allowed attackers to enumerate valid client IDs.
**Learning:** Any validation check (like checking for an empty secret) that occurs *after* a database lookup and results in a skipped cryptographic operation must still perform a dummy operation to balance the execution time.
**Prevention:** Ensure that early returns for invalid credentials (like empty secrets) following a database lookup are paired with `hasher.DummyCompare(ctx)` before returning to prevent timing attacks.
