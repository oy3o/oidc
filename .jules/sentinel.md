...
## 2024-05-18 - [Client Enumeration Timing Attack]
**Vulnerability:** In `AuthenticateClient`, if a confidential client had an empty secret provided, it would return early without performing a hash comparison.
**Learning:** This early return skips the expensive hash operation, allowing an attacker to distinguish between valid and invalid clients by measuring response times, leading to client enumeration.
**Prevention:** Always execute a dummy cryptographic operation (like `DummyCompare`) when a real hash comparison is skipped for authentication endpoints to ensure constant-time responses.
