...
## 2024-05-18 - [Client ID Enumeration via Timing Attack]
**Vulnerability:** In `exchange.go`, during client authentication (`AuthenticateClient`), if a client provided a valid client ID and an empty client secret, the code would exit early before verifying the secret. However, if the client ID was invalid, it performed a dummy hash operation (`hasher.DummyCompare`). This created a timing difference: fast response for valid client IDs with empty secrets, and slow response for invalid client IDs.
**Learning:** This timing difference allows an attacker to brute force and enumerate valid client IDs by measuring response times when sending empty secrets.
**Prevention:** Always perform dummy cryptographic operations to ensure constant-time responses on all failure paths that skip a real hash verification. In this case, `_ = hasher.DummyCompare(ctx)` must be added before the early return for an empty client secret.
