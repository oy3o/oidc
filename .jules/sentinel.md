...
## 2024-06-20 - [DPoP ParseUnverified Validation Bypass and DoS Risk]
**Vulnerability:** DPoP headers were parsed using `jwt.ParseUnverified` without enforcing length bounds, risking CPU/memory exhaustion. The claims were also being read directly using `ParseUnverified` after validation, which could allow maliciously modified tokens to bypass signature checks if the logic mismatches.
**Learning:** Never use `ParseUnverified` to extract claims; always rely on the claims object returned by the cryptographic verification process (`ParseWithClaims`). Also, enforce length limits on DPoP headers to prevent DoS attacks.
**Prevention:** Validate header length before processing, and refactor `VerifyDPoPProof` to return the verified `*DPoPProof` claims instead of just the JKT, ensuring cryptographic validation cannot be bypassed.
