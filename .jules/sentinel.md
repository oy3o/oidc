...
## 2024-05-18 - [DPoP Proof Validation Bypass]
**Vulnerability:** The DPoP middleware incorrectly re-parsed the unverified DPoP header using `jwt.ParseUnverified` to extract the `ATH` (Access Token Hash) claim, even after properly verifying the token via `VerifyDPoPProof`. This allows attackers to forge or alter the `ATH` claim in an unverified token, bypassing cryptographic validation.
**Learning:** Always use the claims extracted directly from the verified token object returned by `jwt.ParseWithClaims`. Do not rely on `ParseUnverified` after validation, as this defeats the purpose of the signature check.
**Prevention:** Ensure that verification functions (like `VerifyDPoPProof`) return the parsed and verified claims object alongside any other required data (e.g., JKT), and utilize those verified claims exclusively in subsequent logic.
