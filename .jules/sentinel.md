...
## 2026-04-02 - [Timing Attack / Client ID Enumeration via Empty Secret]
**Vulnerability:** In `AuthenticateClient`, when a confidential client was retrieved from the database and validated, an early return was executed if the provided `clientSecret` was empty, skipping the secret validation (which involves a time-consuming hash comparison).
**Learning:** This timing difference could allow an attacker to determine if a confidential client requires a secret, or enumerate valid client IDs, since invalid client IDs would take different execution times. Skipping a real hash comparison creates an observable timing discrepancy.
**Prevention:** To prevent client ID enumeration and timing attacks, ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped for an existing client.
