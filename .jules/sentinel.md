...
**Vulnerability:** In `AuthenticateClient`, an empty client secret check after a successful client ID lookup returned immediately without performing a dummy hash comparison, unlike the invalid client ID path. This difference in execution time could allow an attacker to enumerate valid confidential client IDs.
**Learning:** When input validation (such as checking for an empty secret) occurs *after* a database lookup, it must be paired with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to prevent timing attacks.
**Prevention:** Perform dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped for confidential clients, ensuring consistent response times across both valid and invalid client requests.
