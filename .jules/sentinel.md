...
## 2026-03-24 - [Unverified JWT Parsing for DPoP Proofs]
**Vulnerability:** In `dpop_middleware.go`, the middleware previously used `jwt.ParseUnverified` after validation via `VerifyDPoPProof` to extract the `ATH` (Access Token Hash) claim from the DPoP header.
**Learning:** This practice re-parsed the JWT without cryptographic verification and ignored potential errors, which is a known anti-pattern and can lead to bypass of cryptographic validation if the payload isn't strictly correlated with the earlier validation. The fix involved modifying `VerifyDPoPProof` to return the verified `*DPoPProof` directly, eliminating the need to re-parse and use `ParseUnverified`.
**Prevention:** Avoid `ParseUnverified` where possible. Instead, always use the claims returned from the cryptographic verification process.
