...
## 2026-05-29 - [Client Enumeration via Empty Secret Timing Attack]
**Vulnerability:** In `AuthenticateClient`, when a confidential client's `clientSecret` was empty, the function returned immediately after the database lookup without executing a hash comparison.
**Learning:** This introduces a timing attack vector. An attacker can determine if a given `client_id` exists by sending an empty secret. If the client exists, the database lookup succeeds, and the empty secret check returns quickly. If it doesn't exist, it might take a different code path or return a "not found" error, allowing enumeration of valid confidential client IDs based on response times.
**Prevention:** Always pair input validation checks (like an empty secret check) that occur *after* a database lookup with a dummy cryptographic operation (e.g., `DummyCompare`) before returning to balance the execution time.
