...
## 2026-03-21 - [Timing Leak via Missing Hash Check on Empty Client Secret]
**Vulnerability:** In `exchange.go`, when authenticating a confidential client, if the provided `clientSecret` was empty, the function returned immediately without performing a hash comparison. This created a timing side-channel.
**Learning:** Attackers can measure the response time of authentication endpoints. A fast rejection for an empty secret versus a slow rejection for an invalid secret (due to computationally expensive hashing like bcrypt) allows an attacker to enumerate valid Client IDs.
**Prevention:** Ensure constant-time execution paths in authentication flows by executing a dummy hashing operation (e.g., `hasher.DummyCompare(ctx)`) before returning an error when a necessary secret is missing.
