...
## 2026-05-19 - [Client Enumeration via Timing Attack]
**Vulnerability:** A timing attack was possible because validation for an empty `clientSecret` returned early without executing a dummy hash comparison, whereas checking for an invalid `clientID` correctly executed `DummyCompare`.
**Learning:** Returning early on input validation (e.g., empty secret) *after* a database lookup allows attackers to distinguish between an invalid ID and a valid ID missing a secret based on response times.
**Prevention:** Always pair input validation (like empty secret checks) that occurs after a database lookup with a dummy cryptographic operation (like `DummyCompare`) before returning an error to equalize execution time and prevent enumeration.
