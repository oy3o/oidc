...
## 2024-05-24 - [Timing Attack / Client ID Enumeration via Empty Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client had an empty secret provided, the function returned immediately without performing a hash comparison.
**Learning:** This early return creates a timing difference compared to when a non-empty secret is provided and hashed. An attacker could use this timing difference to enumerate valid client IDs even without knowing the secret.
**Prevention:** When input validation (such as checking for an empty secret) occurs *after* a database lookup, it must be paired with a dummy cryptographic operation (like `DummyCompare`) before returning to prevent timing attacks.
