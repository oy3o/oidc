...
## 2025-02-23 - Prevent DPoP Validation Bypass and DoS
**Vulnerability:** The DPoP middleware used `jwt.ParseUnverified` to parse the DPoP header, creating a potential vector for altering unverified claims after validation. Additionally, there were no limits on the size of the DPoP header, allowing for potential CPU and memory exhaustion (DoS).
**Learning:** `ParseUnverified` is dangerous because it bypasses cryptographic validation. Extract claims directly from the cryptographically validated result (`ParseWithClaims`). Always constrain the size of untrusted input to prevent resource exhaustion attacks.
**Prevention:** Avoid `ParseUnverified` for claims extraction; use the parsed claims object returned by `ParseWithClaims`. Implement strict length checking on DPoP headers before parsing.
