...
## 2026-03-22 - [Empty Client Secret Timing Attack]
**Vulnerability:** In `AuthenticateClient`, if a confidential client sent an empty `client_secret`, it returned an error immediately without performing any cryptographic operations, unlike when an invalid secret was provided.
**Learning:** This early return creates a timing oracle that allows an attacker to distinguish between a valid and invalid `client_id`. If the ID exists, the request returns immediately on an empty secret. If it doesn't exist, it spends time doing a dummy comparison (if that was implemented earlier in the flow). This enables client enumeration.
**Prevention:** Always perform a dummy cryptographic operation (like `hasher.DummyCompare`) before returning an error for an empty secret validation check that occurs *after* a database lookup. This ensures a consistent response time regardless of whether the secret was empty or not.
