...
## 2026-03-22 - [Timing Attack via Empty Client Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client exists but the provided `clientSecret` is empty, the code returned early without performing any hash comparison. This created a timing difference compared to providing an incorrect, non-empty secret (which would execute a slow hash check). This allowed attackers to enumerate valid client IDs.
**Learning:** Early returns based on simple string checks (like length == 0) prior to cryptographic operations create side channels that can be exploited for enumeration attacks. We must ensure constant-time responses in authentication endpoints.
**Prevention:** Whenever a real hash comparison is skipped for an existing entity (e.g., due to an empty secret or other validation failure), explicitly perform a dummy operation like `Hasher.DummyCompare(ctx)` to equalise the response time.
