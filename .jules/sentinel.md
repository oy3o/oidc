...
## 2024-05-14 - [Timing Attack via Empty Secret in Client Authentication]
**Vulnerability:** In `AuthenticateClient`, if a confidential client had an empty secret provided, it returned early without performing a hash comparison. This allowed attackers to enumerate valid client IDs by observing faster response times compared to providing a non-empty secret (which triggered a slow hash operation).
**Learning:** Early input validation checks (like empty strings) occurring *after* a database lookup can bypass timing attack mitigations if not paired with a dummy cryptographic operation.
**Prevention:** Always perform a dummy cryptographic operation (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped for an existing client to maintain constant response times.
