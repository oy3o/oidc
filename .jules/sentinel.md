...
## 2026-03-22 - [Client ID Enumeration via Timing Attack on Empty Secret]
**Vulnerability:** In `AuthenticateClient`, if an attacker provided a valid confidential Client ID but an empty `clientSecret`, the code returned early without performing a hash comparison. However, if an invalid Client ID was provided, the code performed a `DummyCompare` which took significantly longer. This timing difference allowed an attacker to enumerate valid confidential Client IDs by sending requests with empty secrets.
**Learning:** Even after successfully retrieving a valid entity from the database, if the entity requires a cryptographic check (like a secret/password hash) and the provided credential is empty, a dummy cryptographic operation MUST still be performed to ensure the response time matches the "entity not found" path. Returning early purely based on an empty secret check after the database lookup creates a measurable timing leak.
**Prevention:** Always perform a `DummyCompare` when skipping a real hash comparison for confidential clients, including cases where the provided secret is empty, to ensure constant-time responses and prevent enumeration attacks.
