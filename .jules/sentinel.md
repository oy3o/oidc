...
## 2026-03-22 - [Client ID Enumeration via Timing Attack]
**Vulnerability:** In `exchange.go`'s `AuthenticateClient`, when a valid confidential client was found but the provided `clientSecret` was empty, the function returned early without performing a hash comparison. This allowed an attacker to enumerate valid `clientID`s by observing the fast response time compared to the delay (e.g., ~100ms for bcrypt) incurred when `DummyCompare` was executed for invalid clients.
**Learning:** Returning early without executing a dummy cryptographic operation on validation failures that bypass actual hash verification creates a timing discrepancy that leaks the validity of the entity (e.g., Client ID).
**Prevention:** Ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped due to input validation failures like empty secrets.
