...
## 2026-03-21 - [Client ID Enumeration / CPU DoS in Client Authentication]
**Vulnerability:** In `AuthenticateClient`, empty `clientSecret` returned early without hashing, allowing timing attacks to enumerate valid client IDs. Unbounded `clientSecret` lengths could cause CPU exhaustion during hashing.
**Learning:** Always apply length limits to inputs processed by expensive cryptographic functions. When masking errors (like missing secrets), ensure a constant-time response by invoking a dummy cryptographic operation.
**Prevention:** Add a length check (e.g., `> 512`) and if the secret is empty or too long, invoke `hasher.DummyCompare(ctx)` before returning the generic error.
