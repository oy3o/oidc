...
## 2026-03-31 - [Client ID Enumeration / Timing Attack]
**Vulnerability:** In `AuthenticateClient`, checking for an empty secret after validating a client ID returned early without performing a hash comparison. This timing discrepancy allowed attackers to distinguish between valid and invalid client IDs using an empty secret.
**Learning:** Timing mitigation strategies must cover all execution paths where an early exit could reveal information about the existence of an entity. If a dummy hash is used to pad time for an invalid entity, a corresponding dummy hash (or real hash) must be executed on all error paths for a valid entity.
**Prevention:** Whenever skipping a cryptographic validation step (like when the provided secret is empty), insert a `DummyCompare` operation to maintain constant-time execution and prevent enumeration attacks.
