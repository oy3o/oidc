...
## 2024-04-10 - [Timing Attack / Client ID Enumeration via Missing DummyCompare]
**Vulnerability:** In `AuthenticateClient`, `ExchangeClientCredentials` and `PasswordGrant`, when early checks failed (such as `clientSecret == ""`), the function returned immediately without executing a dummy cryptographic operation. This created a measurable timing difference between a valid client ID with an empty secret and an invalid client ID (or a client ID with a provided secret), which could be used to enumerate valid Client IDs.
**Learning:** Returning early on simple conditions before executing computationally expensive hash comparisons (e.g., `bcrypt.CompareHashAndPassword`) opens the application to timing attacks. An attacker can discern whether a client exists or not by sending an empty secret.
**Prevention:** To prevent client ID enumeration and timing attacks, ensure that authorization and token exchange endpoints perform constant-time responses by invoking dummy operations (like `Hasher.DummyCompare`) whenever a real hash comparison is skipped or short-circuited.
