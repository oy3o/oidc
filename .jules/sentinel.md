...
## 2024-05-24 - [Timing Attack / Client Enumeration on Empty Secret]
**Vulnerability:** In `AuthenticateClient`, returning early on an empty `clientSecret` for a valid confidential client *after* a successful database lookup created a timing difference.
**Learning:** Attackers can distinguish between invalid clients (where `DummyCompare` is run to simulate hash time) and valid clients with empty secrets (where it returns immediately), allowing client ID enumeration.
**Prevention:** Any input validation check that bypasses a cryptographic operation after the target entity has been identified must perform a dummy operation (e.g., `DummyCompare`) to balance execution time and prevent enumeration.
