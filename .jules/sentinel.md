...
## 2026-04-07 - [Client ID Enumeration via Timing Attack]
**Vulnerability:** In `AuthenticateClient`, if a valid confidential client ID was provided but the `clientSecret` was empty, the function returned immediately without performing a hash comparison. Conversely, if an invalid client ID was provided, the function performed a `DummyCompare` which takes significantly longer. This timing discrepancy allowed an attacker to enumerate valid confidential client IDs by sending requests with empty secrets.
**Learning:** Early returns in authentication flows, even for simple validation checks like empty strings, can introduce timing leaks if they bypass expensive cryptographic operations that are normally executed in other error paths (like client not found).
**Prevention:** Ensure constant-time responses in authentication endpoints by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped due to invalid or missing credentials.
