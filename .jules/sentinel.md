...
## 2024-05-11 - Prevent validation bypass using ParseUnverified
**Vulnerability:** The application used `jwt.ParseUnverified` to extract the `ath` (Access Token Hash) from the DPoP header after verifying it. This is dangerous because it ignores the cryptographically verified claims and blindly trusts the unverified payload, opening a window for validation bypass if token data is maliciously altered.
**Learning:** Always use the claims object returned by the cryptographic verification process (`jwt.ParseWithClaims`) instead of re-parsing the header without verification.
**Prevention:** The `VerifyDPoPProof` function was modified to return the verified `*DPoPProof` claims alongside the JKT, eliminating the need to use `ParseUnverified`.
