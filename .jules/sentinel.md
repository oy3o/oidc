...
## 2024-05-24 - [DPoP Header Length Limit Missing]
**Vulnerability:** The DPoP header in `VerifyDPoPProof` was being parsed as a JWT without any length validation, allowing attackers to potentially exhaust CPU and memory resources by submitting excessively large headers.
**Learning:** Cryptographic and parsing operations, such as `jwt.ParseWithClaims`, should never process unbounded input lengths. Large payloads can significantly slow down processing or cause memory exhaustion, leading to Denial of Service (DoS).
**Prevention:** Enforce strict input length bounds (e.g., a maximum of 4096 bytes) *before* passing user-supplied input to intensive parsing or cryptographic functions.
