...
## 2024-05-24 - [Client Enumeration via Timing Attack on Empty Secret]
**Vulnerability:** In `AuthenticateClient`, if a confidential client was found in the database but the provided client secret was empty, the function returned immediately without performing a dummy cryptographic operation.
**Learning:** This timing difference could allow an attacker to enumerate valid confidential client IDs, because providing an empty secret for a valid client would return quickly, whereas providing an invalid client ID or a non-empty secret would take longer due to the `DummyCompare` or actual hash verification.
**Prevention:** Always perform a dummy cryptographic operation (like `DummyCompare`) when skipping a real hash comparison after a database lookup, ensuring constant-time responses regardless of whether the client exists or the secret is empty.
