...
## 2026-03-22 - [ParseUnverified Validation Bypass & Missing Length Limit in DPoP]
**Vulnerability:** DPoP validation in middleware used `jwt.ParseUnverified` to extract claims instead of using the claims from the securely verified token. Additionally, the DPoP header parsing lacked a length limit, posing a DoS risk.
**Learning:** Using `ParseUnverified` bypasses cryptographic verification, allowing an attacker to supply maliciously altered claims in the unverified portion of the JWT if the application relies on it. Furthermore, processing unbound inputs during cryptographic or parsing operations can lead to resource exhaustion.
**Prevention:** Always rely on the claims object returned by the cryptographic verification process (`VerifyDPoPProof` or `jwt.ParseWithClaims`). Never use `ParseUnverified` for accessing data that drives application logic. Always enforce strict length limits (e.g., 4096 bytes) on DPoP headers before parsing.
