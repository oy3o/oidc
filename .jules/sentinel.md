...
## 2026-03-22 - [Timing Attack via Early Return on Empty Client Secret]
**Vulnerability:** The `AuthenticateClient` function returned instantly if a confidential client was found but the provided `clientSecret` was empty. For a non-existent client, it executed a dummy hash comparison to prevent timing attacks. This discrepancy allowed attackers to enumerate existing confidential clients by submitting empty secrets and observing response times.
**Learning:** Defensive measures like `DummyCompare` must be universally applied across all failure paths that branch *after* the initial lookup to ensure uniform execution times. Early returns that skip expensive operations based on input validation *after* a successful database lookup create side-channel vulnerabilities.
**Prevention:** Ensure that all failure paths for a given operation (e.g., authentication failure due to empty secret vs. authentication failure due to invalid secret) consume the same amount of time by injecting dummy operations where necessary.
