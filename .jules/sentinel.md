...
**Prevention:** Enforce input length bounds (e.g., maximum token length) *before* decoding strings and *always* check for and gracefully handle `err` returned by `DecodeString` routines.## 2026-04-21 - [Timing Attack via Client Enumeration]
**Vulnerability:** In `exchange.go`, `AuthenticateClient` returned early when checking if a confidential client provided an empty secret (`clientSecret == ""`). This allowed attackers to enumerate valid client IDs by measuring response times.
**Learning:** Security functions that perform hash validation must take a constant amount of time to execute. Short-circuiting validation early for bad input (like an empty password or secret) creates a timing side-channel.
**Prevention:** Ensure constant-time responses by performing dummy hash comparisons (e.g., `hasher.DummyCompare(ctx)`) before returning an error in cases where real hash validation is skipped.
