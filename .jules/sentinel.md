...
**Prevention:** Enforce input length bounds (e.g., maximum token length) *before* decoding strings and *always* check for and gracefully handle `err` returned by `DecodeString` routines.## 2025-02-18 - Client ID Enumeration via Timing Attacks on Empty Secrets
**Vulnerability:** A timing side-channel existed in `AuthenticateClient` where confidential clients providing an empty `client_secret` were rejected immediately, whereas those with non-empty, invalid secrets were evaluated via an expensive hashing algorithm.
**Learning:** The discrepancy in response times between immediately rejected (empty secret) and evaluated (hashing process) allowed an attacker to distinguish between these two states, potentially enabling client ID enumeration by monitoring response latency.
**Prevention:** Always perform a constant-time dummy hashing operation (e.g., `Hasher.DummyCompare`) on failure paths that bypass actual secret validation to obscure branching logic and equalize response times. Do not return early without executing a dummy operation if the client identity is valid.
