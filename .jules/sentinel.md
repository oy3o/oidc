...
## 2024-06-10 - [DoS via Unbounded DPoP Header Parsing]
**Vulnerability:** The `VerifyDPoPProof` function extracted the `DPoP` header from incoming HTTP requests and passed it directly to `jwt.ParseWithClaims` without any length validation.
**Learning:** DPoP headers are JWTs. If a malicious client sends an excessively large DPoP header, the JWT parsing library will consume excessive CPU and memory, potentially leading to a Denial of Service (DoS). Cryptographic parsing operations must enforce strict input bounds.
**Prevention:** Enforce a strict length limit (e.g., 4096 bytes) on the `DPoP` header before attempting to parse or validate it cryptographically.
