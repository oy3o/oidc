...
## 2026-03-21 - [JWT Validation Bypass via ParseUnverified]
**Vulnerability:** The `dpop_middleware.go` file used `jwt.ParseUnverified` to extract DPoP proof claims (e.g., `ATH`) after validation had already been performed by `VerifyDPoPProof`. This means the extracted claims in the middleware were not cryptographically verified, creating a bypass if the original validation did not return the exact same parsed object.
**Learning:** `jwt.ParseUnverified` should never be used to extract claims for security decisions. If claims need to be used after validation, they must be returned from the validation function itself (`VerifyDPoPProof`), ensuring that only the cryptographically verified payload is consumed. Also, limiting the DPoP header length protects against excessive resource consumption.
**Prevention:** Always extract and use claims directly from the cryptographically verified token returned by `ParseWithClaims`. Avoid `ParseUnverified` entirely when making access or validation decisions. Also, bound the input length of incoming headers (e.g., DPoP) to a reasonable size to prevent CPU/memory exhaustion.
