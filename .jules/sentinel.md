...
## 2026-03-22 - [DPoP Proof Missing Length Limit]
**Vulnerability:** The DPoP header was parsed as a JWT without first enforcing a maximum length. This allowed an attacker to send an excessively large DPoP proof, potentially causing a CPU and memory Denial of Service (DoS) during the parsing phase.
**Learning:** Any endpoint that accepts and parses user-supplied tokens (like JWTs) must limit the size of the incoming string prior to invoking parsing libraries, as large payloads take significantly longer to hash, parse, and allocate memory for.
**Prevention:** Always enforce a strict length limit (e.g., 4096 bytes) on cryptographic inputs such as JWTs and DPoP headers before parsing them.
