...
## 2024-05-18 - [Client ID Enumeration via Empty Secret Timing Leak]
**Vulnerability:** In `AuthenticateClient`, providing a valid confidential Client ID with an empty `clientSecret` caused an early return without any dummy cryptographic operations. An attacker could measure the response time to distinguish valid Client IDs from invalid ones (which would execute `DummyCompare`).
**Learning:** Early validation returns before expensive operations (like password hashing) must be protected by dummy work to ensure constant-time execution across both valid and invalid input branches.
**Prevention:** Always use `hasher.DummyCompare(ctx)` or an equivalent constant-time delay when bypassing a cryptographic operation (e.g., due to missing input) in an authentication flow.
