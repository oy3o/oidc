...
## 2026-03-21 - [DPoP Header Length Limit & Cryptographic Bypass Prevention]
**Vulnerability:** DPoP headers were parsed using `jwt.ParseUnverified` to extract the `ATH` claim after signature validation, opening a potential vector to bypass validation if malicious claims were crafted. Additionally, DPoP headers were parsed without length limits, posing a DoS risk via memory exhaustion.
**Learning:** Always enforce length bounds on cryptographic inputs (like DPoP headers) before parsing to prevent resource exhaustion. Furthermore, never use `ParseUnverified` to extract claims; always rely strictly on the `claims` object safely returned by the cryptographic verification function (e.g., `VerifyDPoPProof`).
**Prevention:** Enforce input limits and update verification functions to return the securely verified claims object for subsequent logic consumption.
