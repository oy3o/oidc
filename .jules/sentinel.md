...
## 2025-01-20 - Prevent Cryptographic Bypass via ParseUnverified
**Vulnerability:** The code relied on `jwt.ParseUnverified` to extract DPoP and logout token claims. If a token's signature was maliciously altered, `ParseUnverified` would successfully parse the altered payload but completely bypass signature verification, enabling an attacker to forge headers and claims.
**Learning:** Parsing an unverified token in a production handler or testing suite ignores cryptographic signatures. `ParseUnverified` shouldn't be used when cryptographically verified claims are readily available.
**Prevention:** Always rely strictly on claims returned by functions like `jwt.ParseWithClaims`, which mandate a valid signature match prior to returning claims.
