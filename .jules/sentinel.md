...
## 2024-05-19 - [Timing Attack via Empty Client Secret]
**Vulnerability:** In `AuthenticateClient`, when a confidential client provided an empty `clientSecret`, the code immediately returned an error, bypassing the expensive hashing step.
**Learning:** This timing difference allows attackers to easily enumerate valid confidential client IDs by sending an empty secret and checking if the response is fast.
**Prevention:** Always pair input validation that skips expensive database or hashing operations (like empty secret checks) with a dummy cryptographic operation (e.g., `DummyCompare`) to balance timing and prevent enumeration attacks.
