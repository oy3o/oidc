...
## 2024-05-24 - [Timing Oracle via Empty Secret Check]
**Vulnerability:** In `AuthenticateClient`, an early return occurred when a confidential client provided an empty `client_secret` *after* the database lookup, without executing a dummy hash comparison.
**Learning:** This missing dummy operation created a timing oracle. Since invalid client IDs and valid client IDs with non-empty secrets both triggered expensive cryptographic operations (either `DummyCompare` or `ValidateSecret`), an attacker could enumerate valid confidential client IDs by sending empty secrets and looking for exceptionally fast responses.
**Prevention:** When performing input validation (like checking for an empty secret) *after* a database lookup, always pair the early return with a `DummyCompare` to ensure constant-time execution and prevent enumeration attacks.
