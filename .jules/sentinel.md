...
## 2024-05-18 - [Cryptographic Validation Bypass via Unverified JWT Parsing]
**Vulnerability:** After cryptographically verifying a DPoP proof JWT (using `VerifyDPoPProof`), the application was re-parsing the original unverified token header string using `jwt.ParseUnverified` to extract the `ATH` claim.
**Learning:** This introduces a vulnerability where an attacker could provide a token that is successfully verified by the cryptographic validation function, but the subsequent unverified parsing extracts malicious claims from an altered token header (e.g., if multiple headers are present or parsing logic differs). Unverified parsing should never be used on untrusted input, especially when the verified claims are already available.
**Prevention:** Avoid using `jwt.ParseUnverified` to extract token claims. Always rely on the claims object returned by the cryptographic verification process (like `jwt.ParseWithClaims`) to prevent validation bypass.
