...
## 2024-06-02 - [Timing Attack / Client Enumeration via Empty Secret]
**Vulnerability:** In `AuthenticateClient`, returning early when a confidential client is requested with an empty secret bypassed the expensive hashing operation. Since this occurred *after* retrieving the client from the database, it created a timing differential allowing an attacker to enumerate valid confidential client IDs without knowing a valid secret.
**Learning:** Even input validation (like checking for an empty string) can introduce a timing vulnerability if it occurs conditionally after a state-dependent operation (like a database lookup) and skips an expected expensive operation.
**Prevention:** Always pair early returns that skip hashing (after a successful client lookup) with a dummy cryptographic operation (like `DummyCompare`) to normalize execution times and prevent timing attacks.
