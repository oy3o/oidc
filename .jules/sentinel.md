...
**Prevention:** Enforce input length bounds (e.g., maximum token length) *before* decoding strings and *always* check for and gracefully handle `err` returned by `DecodeString` routines.## 2025-02-23 - Prevent timing attack on empty secret validation
**Vulnerability:** In `AuthenticateClient`, if a confidential client provided an empty `clientSecret`, the function returned an error immediately, skipping the hash comparison (`client.ValidateSecret`). This observable timing difference allowed an attacker to determine if a given `clientID` was valid and confidential by sending an empty secret and measuring the response time.
**Learning:** Early return statements in authentication flows create timing discrepancies. Even if a prior database lookup timing is masked (e.g., handling `ErrClientNotFound` with `DummyCompare`), secondary validation steps (like checking for an empty secret before hashing) can re-introduce the vulnerability on different paths.
**Prevention:** Ensure constant-time responses for all failure paths in authentication functions. Apply `DummyCompare` (or equivalent dummy work) whenever a real, expensive operation (like hash validation) is skipped due to invalid input, *after* the initial data fetch.
