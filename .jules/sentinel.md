...
## 2026-03-22 - [DPoP Proof Validation Bypass via jwt.ParseUnverified]
**Vulnerability:** In `dpop_middleware.go`, `VerifyDPoPProof` properly verified the DPoP signature and returned the `jkt`, but then `DPoPMiddleware` used `jwt.ParseUnverified` again on the `dpopHeader` to extract the `ATH` claim. This bypasses cryptographic validation for the `ATH` claim because it relies on the unverified, potentially altered token body. Furthermore, DPoP headers had no length limits before parsing.
**Learning:** Never use `jwt.ParseUnverified` to extract claims that have already been (or should be) cryptographically verified. Malicious actors could supply a valid signature for the header but alter the body containing `ATH`, leading to validation bypass. Also, unbounded input length opens up CPU and memory exhaustion (DoS) attacks.
**Prevention:** Instead of re-parsing headers using `ParseUnverified`, modify the verification function (e.g., `VerifyDPoPProof`) to return the cryptographically verified claims object alongside other verified outputs. Additionally, ensure a strict length limit (e.g., 4096 bytes) is enforced on DPoP headers before passing them to parsing functions like `jwt.ParseWithClaims`.
