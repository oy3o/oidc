...
## 2024-03-24 - [Unverified JWT Parsing and DPoP DoS]
**Vulnerability:** DPoP headers were parsed using `jwt.ParseUnverified` to extract the `ATH` claim, bypassing proper cryptographic validation, which is dangerous if unverified claims are trusted. Furthermore, the DPoP header parsing lacked a length limit, allowing attackers to send massive headers resulting in CPU and memory exhaustion (DoS).
**Learning:** `jwt.ParseUnverified` should never be used, as it trusts potentially attacker-controlled data without validation. Instead, the validated claims from `jwt.ParseWithClaims` should be returned and used. To prevent resource exhaustion, all cryptographic and decoding inputs (like headers or tokens) must have strict length bounds enforced before processing.
**Prevention:** Remove `ParseUnverified` usage entirely, return the validated claims pointer `*DPoPProof` from `VerifyDPoPProof`, and enforce a max length limit (e.g., 4096 bytes) on the raw DPoP header string prior to parsing.
