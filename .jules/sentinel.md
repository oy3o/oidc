...
## 2024-06-30 - Fix JWT validation bypass in DPoP middleware
**Vulnerability:** The DPoP middleware used `jwt.ParseUnverified` to parse the DPoP header just to extract the `ATH` (Access Token Hash) claim, even though the signature and claims had already been fully verified using `jwt.ParseWithClaims` inside `VerifyDPoPProof`. Using `jwt.ParseUnverified` exposes the application to security bypasses if attackers manipulate unverified token data, and introduces unnecessary security risk and overhead.
**Learning:** Extracting claims from unverified parses instead of using the result of the cryptographically verified parsing process bypasses the security guarantees of JWTs. Verified claims should be returned and used directly.
**Prevention:** Always rely on the claims object returned by the cryptographic verification process (`jwt.ParseWithClaims`). Do not re-parse unverified JWTs just to extract specific fields. We refactored `VerifyDPoPProof` to return the verified `*DPoPProof` struct, and consumed it in `dpop_middleware.go` directly.
