...
## 2024-06-05 - Avoid using ParseUnverified to re-parse validated DPoP headers
**Vulnerability:** The DPoP middleware `dpop_middleware.go` was extracting claims (`ATH`) from the DPoP proof header by calling `ParseUnverified` on the raw header token *after* `VerifyDPoPProof` had already correctly parsed and verified the token. `ParseUnverified` blindly decodes the token claims without performing cryptographic validation.
**Learning:** Even though the signature is verified before, double-parsing introduces drift and is a bad practice. An attacker could potentially cause divergence in what parser 1 (with validation) sees versus parser 2 (unverified), or simply using `ParseUnverified` might be flagged as a critical flaw by scanners.
**Prevention:** `VerifyDPoPProof` should return the verified, unmarshalled `*DPoPProof` object, and the middleware should extract claims directly from this returned object instead of calling `ParseUnverified`.
