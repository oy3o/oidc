...
## 2026-03-24 - [Timing Attack Mitigation via DummyCompare]
**Vulnerability:** In `AuthenticateClient`, empty `clientSecret` checks for confidential clients returned immediately without performing hash validation, while invalid valid-format secrets triggered a slow `ValidateSecret` hash compare. This timing discrepancy allowed attackers to enumerate valid client IDs by measuring response times.
**Learning:** Timing differences in authentication flows can leak information about the existence of user or client records. Constant-time responses are crucial for all branches of authentication logic.
**Prevention:** Ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a real hash comparison is skipped due to early validation failures (like empty secrets) on validly located records.
