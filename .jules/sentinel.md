...
**Vulnerability:** In `AuthenticateClient`, if a valid confidential client was requested with an empty `clientSecret`, the function returned immediately. However, if an invalid client ID was requested, a dummy hash comparison was performed. This timing difference allowed attackers to enumerate valid confidential client IDs.
**Learning:** Returning early on validation errors *after* a database lookup without performing a dummy cryptographic operation creates a timing oracle. This undermines the protection intended by dummy operations on the "not found" path.
**Prevention:** Always pair input validation (like empty secret checks) that occurs after a database lookup with a dummy cryptographic operation (e.g., `DummyCompare`) before returning, to equalize execution time and prevent enumeration.
