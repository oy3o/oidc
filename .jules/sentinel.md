...
## 2024-06-01 - [DPoP Validation Bypass via ParseUnverified]
**Vulnerability:** The `DPoPMiddleware` relied on `jwt.ParseUnverified` to extract the `ATH` claim from the incoming DPoP proof. This meant an attacker could tamper with the unsigned header payload, completely bypassing cryptographic validation, and allowing arbitrary `ATH` values to be injected.
**Learning:** Never use `ParseUnverified` to extract security-critical claims. You must always use the claims object that was outputted by the cryptographic verification function (like `ParseWithClaims`) to ensure data integrity and authenticity.
**Prevention:** The `VerifyDPoPProof` function should directly return the cryptographically verified `DPoPProof` object, which should then be used by the caller to extract needed claims (e.g., `ATH`), completely eliminating the unsafe use of `ParseUnverified`.
