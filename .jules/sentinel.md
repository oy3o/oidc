...
## 2024-05-18 - [Client Enumeration via Timing Side-Channel]
**Vulnerability:** In `exchange.go`'s `AuthenticateClient`, if a confidential client was requested with an empty `clientSecret`, the code returned `ErrInvalidClient` immediately. However, if an invalid `clientIDStr` was provided, the code called `hasher.DummyCompare(ctx)` during `GetAndVerifyClient` before returning an error. This timing difference allowed attackers to enumerate valid client IDs by intentionally sending an empty secret.
**Learning:** Returning early on simple validation checks (like empty strings) without performing a dummy operation when the alternative path involves expensive cryptographic operations (like hashing or database lookups) creates a timing side-channel.
**Prevention:** Ensure that all error paths in authentication flows take a similar amount of time. When returning early due to invalid or missing credentials (like an empty secret), execute a dummy cryptographic operation (`hasher.DummyCompare`) to mask the timing difference.
