...
## 2024-05-18 - [DPoP Header Length Limit Missing / DoS]
**Vulnerability:** DPoP headers obtained from HTTP requests were directly passed to `jwt.ParseWithClaims` without any length validation.
**Learning:** Parsing large, untrusted input strings as JWTs can lead to CPU and memory exhaustion (DoS attacks) because cryptographic parsing and decoding are resource-intensive.
**Prevention:** Enforce a strict length limit (e.g., 4096 bytes) on DPoP headers before passing them to parsing functions like `jwt.ParseWithClaims` or similar JWT processors.
