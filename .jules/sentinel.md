...
## 2024-05-30 - [Client Enumeration Timing Attack on Empty Secret]
**Vulnerability:** When a client is authenticated via `AuthenticateClient`, if the client is confidential and `clientSecret == ""` (empty secret), the function returned an error immediately without performing a dummy hash comparison.
**Learning:** An attacker could exploit this timing difference to enumerate valid confidential client IDs because an empty secret results in a fast failure, whereas evaluating a valid secret invokes the hash validation process.
**Prevention:** Always enforce dummy hash evaluation on empty secrets or other early return conditions in `AuthenticateClient` when authenticating confidential clients, similar to how it is handled for invalid clients, to normalize execution time.
