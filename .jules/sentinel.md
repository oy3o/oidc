...
## 2024-05-20 - [DPoP Proof Validation Bypass]
**Vulnerability:** The DPoP middleware was parsing the DPoP header using `jwt.ParseUnverified` to extract the `ATH` (Access Token Hash) claim, even after `VerifyDPoPProof` had already successfully validated the token and signature. This allowed an attacker to potentially supply a valid DPoP proof with manipulated claims that wouldn't be caught since `ParseUnverified` bypasses cryptographic signature verification. Additionally, the same vulnerable pattern was present in `session_test.go` for logout token claims validation.
**Learning:** Never use `jwt.ParseUnverified` to extract claims from security tokens. Always rely on the claims object returned by the cryptographic verification process (`jwt.ParseWithClaims`).
**Prevention:** Always ensure the verification function (`VerifyDPoPProof` or `jwt.ParseWithClaims`) directly returns the validated claims object, and use that object for downstream logic instead of re-parsing the token unverified.
