...
## 2024-05-24 - Timing Attack via Early Return on Empty Secret
**Vulnerability:** The `AuthenticateClient` function in `exchange.go` returned immediately with an error if a confidential client was provided with an empty `clientSecret`. This allowed an attacker to distinguish between a non-existent client ID (which triggered a slow dummy hash compare) and an existing client ID with an empty secret (which returned instantly).
**Learning:** Early returns based on input validation (like empty secret checks) *after* successful entity lookup can inadvertently bypass timing attack mitigations (like dummy hash comparisons) that exist elsewhere in the flow. The absence of a hash check still leaks state if not masked.
**Prevention:** Ensure that all failure paths for authentication that skip the actual cryptographic hash comparison invoke a dummy hash comparison to maintain a constant-time response profile and prevent enumeration.
