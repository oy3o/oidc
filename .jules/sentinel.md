...
## 2026-04-25 - [Timing Attack via Early Return on Empty Secret]
**Vulnerability:** In `exchange.go`'s `AuthenticateClient` function, checking for an empty secret returned immediately without performing a dummy hash comparison.
**Learning:** Returning early on an empty secret check without a dummy operation creates a measurable timing difference. Attackers can exploit this to distinguish between an invalid client secret and an empty one, potentially facilitating enumeration attacks. Even early validation checks must take constant time when they bypass cryptographic operations.
**Prevention:** Always perform a dummy hash operation (e.g., `hasher.DummyCompare(ctx)`) whenever skipping a real hash comparison, even for early input validation like empty secret checks.
