...
## 2026-05-26 - [Client ID Enumeration / Timing Attack via Missing Secret]
**Vulnerability:** In `exchange.go`, `AuthenticateClient` returned early when a valid confidential client ID was provided but the `clientSecret` was empty, skipping the expensive hash validation. This timing difference could allow attackers to enumerate valid client IDs.
**Learning:** Even when input validation (like an empty secret check) fails, if it occurs *after* a database lookup that would normally be followed by an expensive cryptographic operation, we must execute a dummy cryptographic operation to normalize the response time.
**Prevention:** Always pair input validation failures that bypass expected cryptographic operations with `DummyCompare` to prevent timing attacks and client enumeration.
