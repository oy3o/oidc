...
## 2024-05-20 - [Client ID Enumeration / Timing Attack]
**Vulnerability:** In `exchange.go`, when an empty `clientSecret` was provided for a confidential client, the code returned early with `ErrInvalidClient` instead of performing a hash comparison.
**Learning:** This allowed an attacker to measure response times and distinguish between valid and invalid client IDs by sending empty secrets, as valid client IDs would return much faster without executing the expensive hash operation.
**Prevention:** To prevent enumeration and timing attacks, always ensure constant-time responses by performing dummy operations (e.g., `Hasher.DummyCompare`) whenever a computationally expensive real hash comparison is skipped or short-circuited.
