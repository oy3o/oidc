...
## 2024-05-13 - Empty Secret Timing Attack in AuthenticateClient
**Vulnerability:** In `exchange.go:AuthenticateClient`, a confidential client with an empty `clientSecret` returned an error immediately without performing a hash comparison (`hasher.DummyCompare`), causing a timing difference that allows an attacker to enumerate valid vs invalid confidential client IDs.
**Learning:** Early validation checks that skip cryptographic operations for absent secrets still leak information. Consistency in response time is required whenever a real hash comparison is skipped for a valid confidential client ID.
**Prevention:** Always execute `hasher.DummyCompare(ctx)` when skipping secret validation (e.g., due to an empty secret) for confidential clients, ensuring timing is consistent with failed authentication attempts.
