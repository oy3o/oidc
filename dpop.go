...
func VerifyDPoPProof(
	...
	// [Security] Limit DPoP header length to prevent CPU/Memory exhaustion attacks (DoS)
	// The reasonable size for a JWT header, payload, and signature typically does not exceed 4096 bytes
	if len(dpopHeader) > 4096 {
		return "", fmt.Errorf("%w: DPoP header too large", ErrInvalidRequest)
	}
