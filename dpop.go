...
func VerifyDPoPProof(
	...
	// [Security] Enforce a strict length limit on the DPoP header to prevent CPU/memory exhaustion (DoS) attacks
	// during JWT parsing and validation.
	if len(dpopHeader) > 4096 {
		return "", fmt.Errorf("%w: DPoP header exceeds maximum allowed length", ErrInvalidRequest)
	}
